...
const Navbar = (props) => {
    ...
    return (
        <nav className="fixed top-0 left-0 right-0 bg-[#23354E] px-5 py-1 lg:py-1 h-8.2 lg:h-18 md:h-17 flex justify-between items-center z-10">
            ...
            {/* Regiter button */}
            {props.currentPage === 'landing' && <button className="hidden lg:block pt-1 pb-2 right-0 px-5 py-4 rounded-lg mt-2 mb-2 mr-2 text-[#071E22] bg-[#D4DFC7] hover:text-white hover:bg-[#22C3FF]">
                <Link to={"/registeration"} className="font-semibold text-xl">Register</Link>
            </button>}
            ...
            <div className={`${isOpen ? "block" : "hidden"} lg:hidden absolute top-8 md:top-10 left-0 right-0 bg-[#23354E] text-center pb-4`}>
                ...
                {props.currentPage === 'landing' && <a href="/#faq" className="block text-[#22C3FF] font-bold text-base mb-2 hover:text-[#D4DFC7]" onClick={() => { setIsOpen(!isOpen); }} >FAQ</a>}
                ...
                {props.currentPage === 'landing' && <button onClick={() => { setIsOpen(!isOpen); }} className="pb-1 px-2 rounded-lg text-[#071E22] bg-[#D4DFC7] hover:text-white hover:bg-[#22C3FF]">
                    <Link to={"/registeration"} className="text-[#071E22] font-medium text-base">Register</Link>
                </button> }
